...
| `customer_segments` | `customer_unique_id` | `customer_unique_id` | Built from `rfm_summary` via KMeans (K=4) (Task 3.3); same 93,358 rows as `rfm_summary` (no exclusions). Adds `cluster` (raw KMeans id, arbitrary/unstable across reruns) and `cluster_label` (human-readable, rank-derived — see `src/models/segmentation.py`). 4 segments: `Loyal / Repeat Buyers` (2,801, frequency>=2, validated 100% match), `High-Value Active` (27,872), `Mid-Value Dormant` (27,001), `Low-Value Active` (35,684) |
