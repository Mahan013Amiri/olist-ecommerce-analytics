...
### `docs/progress_log.md` (جدید)
یک فایل ساده که بعد از تکمیل هر Task، یک بخش کوتاه بهش اضافه می‌شود: چه کاری انجام شد، چه فایل‌هایی ساخته/تغییر کردند، و مهم‌ترین یافته‌ها یا تصمیم‌ها. هدفش جدا نگه‌داشتن جزئیات اجرا از README است — README باید مختصر و برای معرفی سریع پروژه بماند؛ جزئیات کامل هر Task جای دیگری ثبت می‌شود.
---
...
└── docs/
    ...
    └── progress_log.md         # جدید — گزارش پیشرفت هر Task (چیکار شد، چه یافته‌هایی)
```
